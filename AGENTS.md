...
This repository is a TypeScript pnpm workspace. It publishes exactly three self-contained packages:

- `@trace-glow-sdk/browser`: public browser SDK with all required private modules bundled.
- `@trace-glow-sdk/vue`: public Vue 3 SDK with browser instrumentation and Vue error integration bundled; Vue remains a peer dependency.
...
- `@trace-glow/core`: runtime-neutral event contract, lifecycle, plugins, bounded queue, batching, sampling, retry, and shutdown.
- `@trace-glow/context`: user, tags, extras, release/environment, and correlation context.
- `@trace-glow/transport`: HTTP, gzip, and Beacon delivery.
- `@trace-glow/logger`: structured, severity-filtered logging.
- `@trace-glow/browser`: browser errors, rejected promises, resource failures, performance, Fetch, and XHR instrumentation.
- `@trace-glow/vue`: Vue application error-handler installation, delegation, and teardown.
- `@trace-glow/node`: process/runtime monitoring, async request context, and HTTP/Express/Koa/Nest middleware.
...
- The repository uses Changesets and links the four public package versions. Do not publish to npm, create a release, or change the `@trace-glow-sdk` scope without explicit user authorization.
