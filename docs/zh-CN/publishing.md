...
1. 确认 npm 账户或组织拥有 `@trace-glow-sdk` scope。仅发布 `@trace-glow-sdk/browser` 和 `@trace-glow-sdk/node` 两个公开包。
...
私有 `@trace-glow/*` 模块只会被内联，不会单独发布。修改私有模块时，仍必须为行为受到影响的 browser 或 Node.js 公开包添加 Changeset。
...
检查两个公开包的 tarball，确认浏览器产物没有引用 Node.js 内置模块，并且两个 tarball 都没有声明对私有 `@trace-glow/*` workspace 包的运行时依赖。
